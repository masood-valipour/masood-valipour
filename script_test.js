...
async function loadData() {
    try {
        ...
        // Extract unique Gamma and Delta values from the primary dataset
        ...
        document.getElementById('param-display').textContent = 'Error loading data files: ' + error.message;
    }
}
...
function updatePlots() {
    ...
    // Compute Optimum probabilities (Purely analytical, depends only on Gamma_e/delta_a)
    ...
    if (!row_UnEnt) {
        console.warn("No UnEnt data found for current parameters");
        return;
    }
    ...
    if (!row_Ent) {
        console.warn("No Entangled data found for current parameters");
        return;
    }
